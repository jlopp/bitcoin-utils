...
while ($height < $maxBlockHeight) {
	...
	foreach ($block->get('tx') as $transaction) {
		// Ignore transactions with over 1 output
		if (count($transaction['vout']) > 1) {
			continue;
		}

		// Look for large consolidations
		if (count($transaction['vin']) < 190) {
			continue;
		}
		...
		if (isset($spamBlockValues[$height])) {
			$spamBlockValues[$height] += $outputValue;
		} else {
			$spamBlockValues[$height] = $outputValue;
		}
	}
